...
const buildSegments = (userId, { type, status, accountId, startDate, endDate, page, limit, sort } = {}) => {
...
const buildFilter = async (userId, { type, status, accountId, startDate, endDate } = {}) => {
...
const fetchTransactions = async (filter, userId, { page, limit, sort } = {}) => {
  ...
  const enrichedData = data.map(tx => {
    ...
    let isUserSender = fromAccountUser &&
      fromAccountUser.toString() === userId.toString();
    let isUserReceiver = toAccountUser &&
      toAccountUser.toString() === userId.toString();
...
const getTransactionsCached = async (userId, options = {}, ttlSeconds = DEFAULT_TTL) => {
...
const getUserTransactions = async (userId, options = {}) => getTransactionsCached(userId, options);

const getTransactionsByFilters = async (userId, filters = {}) => getTransactionsCached(userId, filters);
